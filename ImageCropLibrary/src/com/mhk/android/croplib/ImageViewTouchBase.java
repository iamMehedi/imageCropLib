...
abstract class ImageViewTouchBase extends ImageView {
    ...
    void fixForZoom()
    {
        ...
        m.getValues(mp);
		float transX = mp[Matrix.MTRANS_X];
		float transY = mp[Matrix.MTRANS_Y];
        float height = rect.height();
        float width  = rect.width();
        ...
        deltaY = getFixTrans(transY, viewHeight, height);
    ...
    protected void dragBy(float dx, float dy)
    {
    	/*final Bitmap bitmap = bitmapDisplayed.getBitmap();
        if (bitmap == null) {
            return;
        ...
        float deltaX=getFixDragTrans(dx, viewWidth, width);
        float deltaY=getFixDragTrans(dy, viewHeight, height);
